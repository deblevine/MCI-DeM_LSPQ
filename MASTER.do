...
Updated by Rachael Whitney, PhD: 01/22/2020
...
keep mci_dem_dyadsubject_id patient_status moca dyadid;
save `tracker';
/* NOTE
...
keep mci_dem_dyadsubject_id sex age sp_age race relationship_sp otherrelationship_specify moca dyadid pnid;
...
label define `p' 1 "I want treatment" 0 "I do not want treatment";
label values `p' `p';
};
...
label define `s' 1 "I want treatment" 0 "I do not want treatment";
label values `s' `s';
};
...
************************************ SAVE OPTION 1: MASTER DATASET UNSTRUCTURED;
*save "S:\Intmed_Rsrch2\GenMed\Restricted\MCI DeM\Study 3\Data Analysis\2020 Freeze\MCIDeM_Study3_Master_200122.dta", replace;

/* 
////////////////////////////////////////////////////////////////////////////////
\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
************************** NOTE TO MOHAMMED & ANDRZEJ **************************
There is a significant amount of missing data in the current dataset, as it is
structured. I anticipate that this will create some analytic issues (especially
for the LSPQ study, given that we want to adjust for patient and study partner
characteristics within the same analysis.) The code below reorganizes the data
such that each line represents a unique dyadid (formerly there were two entries
per dyadid: one for the patient and one for the partner). Variables now end in
suffixes that provide information on respondent type (patient | partner) and 
person being referenced (self | dyad-mate):
...
Let me know your thoughts and whether you think this organization should apply:
1. To the analytic dataset for LSPQ analyses only, or: 2. To the entire master
data file. Use save option 1 (commented out above) if the former and save option 
2 (commented out at the end of the syntax) if the latter. NOTE: if we don't want
to alter the master substantially (option 1), some of the code that follows will
need to be moved up in the syntax (e.g., certain recodes and renames). 
********************************************************************************
\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
////////////////////////////////////////////////////////////////////////////////
*/
...
*** _PT & _PTR VARIABLES;
quietly{;
label define dsdm 1 "I prefer to make the decision about which treatment I will receive" 2 "I prefer to make the final decision about my treatment after seriously considering my doctors opinion" 3 "I prefer that my doctor and I share responsibility for deciding which treatment is best for me" 4 "I prefer that my doctor makes that final decision about which treatment will be used but seriously considers my opinion" 5 "I prefer to leave all decisions regarding treatment to my doctor";
label define education 1 "8th grade or less" 2 "Some high school, but did not graduate" 3 "High school graduate or GED" 4 "Trade school" 5 "Some college or 2-year college degree" 6 "4-year college degree" 7 "More than 4-year college degree";
label define marital 1 "Married " 2 "Living with someone as a couple, but not married" 3 "Divorced or separated" 4 "Widowed" 5 "Never married";
...
label define sdm 1 "I definitely want to be involved in making the decision" 2 "I probably want to be involved in making the decision" 3 "I am unsure" 4 "I probably do not want to be involved in making the decision" 5 "I definitely do not want to be involved in making the decision";
label define antibiotics 1 "I definitely want antibiotics" 2 "I probably want antibiotics" 3 "I am unsure" 4 "I probably do not want antibiotics" 5 "I definitely do not want antibiotics";
label define cpr 1 "I definitely want CPR" 2 "I probably want CPR" 3 "I am unsure" 4 "I probably do not want CPR" 5 "I definitely do not want CPR";
label define gallbladder 1 "I definitely want surgery" 2 "I probably want surgery" 3 "I am unsure" 4 "I probably do not want surgery" 5 "I definitely do not want surgery";
label define artificial_fd 1 "I definitely want artificial feeding and fluids" 2 "I probably want artificial feeding and fluids" 3 "I am unsure" 4 "I probably do not want artificial feeding and fluids" 5 "I definitely do not want artificial feeding and fluids";
...
foreach var in `lspq' {;
label values `d'_`var'_`t' `var';
};
};
*SDM IN AMI, STROKE;
foreach var in `sdm' {;
label values `var'_`t';
label values `var'_`t' sdm;
};
*RISK PERCEPTION;
foreach var in `risk' {;
label values `var'_`t';
...
*** _APT VARIABLES ONLY;
quietly{;
label define memory_apt 1 "Normal memory" 2 "Occasionally forget things that they were told recently. Does not cause many problems." 3 "Mild consistent forgetfulness. Remembers recent events but often forgets parts." 4 "Moderate memory loss. Worse for recent events. May not remember something they were just told. Causes problems with everyday activities." 5 "Substantial memory loss. Quickly forgets recent or newly learned things. Can only remember things that they have known for a long time." 6 "Does not remember basic facts like the day of the week, when last meal was eaten, or what the next meal will be." 7 "Does not remember even the most basic things."; 
label values memory_apt memory_apt;
label define speech_and_language_apt 1 "Normal ability to talk and to understand others." 2 "Sometimes cannot find a word but able to carry on conversations." 3 "Often forgets words. May use the wrong word in its place. Some trouble expressing thoughts and giving answers." 4 "Usually answers questions using sentences but rarely starts a conversation." 5 "Answers questions but responses are often hard to understand or dont make sense. Usually able to follow simply instructions." 6 "Speech often does not make sense. Cannot answer questions or follow instructions." 7 "Does not respond most of the time."; 
label values speech_and_language_apt speech_and_language_apt; 
label define reg_of_fam_members_apt 1 "Normal. Recognizes people and generally knows who they are." 2 "Usually recognizes grandchildren, cousins, or relatives who are not seen frequently but may not recall how they are related." 3 "3.Usually does not recognize family members who are not seen frequently. Is often confused about how family members such as grandchildren, nieces, or nephews are related to them." 4 "Sometimes does not recognize close family members or others whom they frequently see. May not recognize their children, brothers, or sisters who are not seen on a regular basis." 5 "Frequently does not recognize spouse or caregiver." 6 "No recognition or awareness of the presence of others."; 
label values reg_of_fam_members_apt reg_of_fam_members_apt;
label define orientation_to_time_apt 1 "Normal awareness of time of day and day of week." 2 "Some confusion about what time it is or what day of the week but not severe enough to interfere with everyday activities." 3 "Frequently confused about time of day." 4 "Almost always confused about time of day." 5 "Seems completely unaware of time.";
label values orientation_to_time_apt orientation_to_time_apt; 
label define orientation_to_place_apt 1 "Normal awareness of where they are, even in new places." 2 "Sometimes disoriented in new places." 3 "Frequently disoriented in new places." 4 "Usually disoriented, even in familiar places. May forget that they are already at home." 5 "Almost always confused about place.";
label values orientation_to_place_apt orientation_to_place_apt; 
label define decisions_apt 1 "Normal. As able to make decisions as before." 2 "Only some difficulty making decisions that arise in day-to-day life." 3 "Moderate difficulty. Gets confused when things get complicated or plans change." 4 "Rarely makes any important decisions. Gets confused easily." 5 "Not able to understand what is happening most of the time.";
label values decisions_apt decisions_apt; 
label define social_and_community_apt 1 "Normal. Acts the same as before with people." 2 "Only mild problems that are not really important but clearly acts differently from previous years." 3 "Can still take part in community activities without help. May appear normal to people who dont know them." 4 "Often has trouble dealing with people outside the home without help from caregiver. Usually can participate in quiet home activities with friends. The problem is clear to anyone who sees them." 5 "No longer takes part in any real way in activities at home involving other people. Can only deal with the primary caregiver." 6 "Little or no response even to primary caregiver.";
label values social_and_community_apt social_and_community_apt;
label define activities_and_respons_apt 1 "Normal. No decline in ability to do things around the house." 2 "Some problems with home activities. May have more trouble with money management (paying bills) and fixing things. Can still go to a store, cook, or clean. Still watches TV or reads a newspaper with interest and understanding." 3 "Makes mistakes with easy tasks like going to a store, cooking, or cleaning. Losing interest in the newspaper, TV, or radio. Often cant follow a long conversation on a single topic." 4 "Not able to shop, cook, or clean without a lot of help. Does not understand the newspaper or the TV. Cannot follow a conversation." 5 "No longer does any home-based activities."; 
label values activities_and_respons_apt activities_and_respons_apt; 
label define personal_care_apt 1 "Normal. Takes care of self as well as they used to." 2 "Sometimes forgets to wash, shave, comb hair, or may dress in wrong type of clothes. Not as neat as they used to be." 3 "Requires help with dressing, washing, and personal grooming." 4 "Totally dependent on help for personal care.";
label values personal_care_apt personal_care_apt; 
label define eatinglevl_apt 1 "Normal. Does not need help in eating food that is served to them." 2 "May need help cutting food or have trouble with some foods but basically able to eat by themselves." 3 "Generally able to geed themselves but may require some help. May lose interest during the meal." 4 "Need to be fed. May have trouble swallowing.";
label values eatinglevl_apt eatinglevl_apt; 
label define urination_and_bowels_apt 1 "Normal. Does not have problems controlling urination or bowels except for physical problems." 2 "Rarely fails to control urination (generally less than one accident per month)." 3 "Occasional failure to control urination (about once a week or less)." 4 "Frequently fails to control urination (about once a week or less)." 5 "Generally fails to control urination and frequently cannot control bowels.";
label values urination_and_bowels_apt urination_and_bowels_apt;
label define place_to_place_apt 1 "Normal. Able to get around on their own. (May have physical problems that require a cane or walker.)" 2 "Sometimes gets confused when driving or taking public transportation, especially in new places. Able to walk places alone." 3 "Cannot drive or take public transportation alone, even in familiar places. Can walk alone outside for short distances. Might get lost if walking too far from home." 4 "Cannot be left outside alone. Can get around the house without getting lost of confused." 5 "Gets confused and needs help finding their way around the house." 6 "Almost always in a bed or chair. May be able to walk a few steps with help but lacks sense of direction." 7 "Always in bed. Unable to sit or stand.";
...
********* SAVE OPTION 2: MASTER DATASET STRUCTURED OR SEPERATE ANALYTIC DATASET;
*save "S:\Intmed_Rsrch2\GenMed\Restricted\MCI DeM\Study 3\Data Analysis\2020 Freeze\MCIDeM_Study3_Master_200122.dta", replace;
*save "S:\Intmed_Rsrch2\GenMed\Restricted\MCI DeM\Study 3\Data Analysis\2020 Freeze\MCIDeM_Study3_LSPQ_Analytic_200122.dta", replace;
